...
def add():
    try:
        ...
        list(map(lambda x: x.get_id(), CarTaxParam.query.all()))
        regions = list(map(lambda x: x.get_id(), Region.query.all()))
        if city_id not in regions:
            return {'reason': 'No such id or rate...'}, 400
        else:
            new_data = CarTaxParam(id, city_id, from_hp_car, to_hp_car, from_production_year_car,
        ...
        return {"message": f"Database error: {str(error)}"}, 500
...
def update():
    ...
    if city_id not in regions:
        ...
        CarTaxParam.query.filter_by(id=id).update(
